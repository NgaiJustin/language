...
_OUTPUT_PATH = flags.DEFINE_string(
    "output_path", None, help="Path where the datasets will be created.", required=True
)
...
_SEED = flags.DEFINE_integer("seed", 42, help="Seed for building few shot examples.")
...
_SHOTS = flags.DEFINE_list(
    "shots", ["0", "1", "5"], help="Number of examples to include in the prompt."
)
...
def _word_candidates(idx: int) -> Sequence[Mapping[str, str]]:
    ...
    with tf.io.gfile.GFile(_WORDS_PATH.value, "r") as f:
        for line in f:
            ...
            if idx < (
                float(score) - _MIN_LL.value
            ) * factor <= idx + 1 and utils.is_candidate_new(candidate):
...
def main(argv: Sequence[str]) -> None:
    ...
    counter = 0
    total = len(sources) * len(create_word_fns) * len(strategies) * len(_SHOTS.value)
    ...
    for create_word_name, create_word_fn in create_word_fns.items():
        ...
        for src_idx, (source_name, source) in enumerate(sorted(sources.items())):
            ...
            logging.info(
                "Output %s_%s examples %d.",
                source_name,
                create_word_name,
                2 * len(winodict),
            )
            file_path = os.path.join(
                _OUTPUT_PATH.value, f"{source_name}_{create_word_name}"
            )
            ...
            for strategy_name, strategy in strategies.items():
                ...
                for shots in _SHOTS.value:
                    utils.write_prompt_files(
                        os.path.join(task_path, f"{task}_{shots}shot"),
                        winodict,
                        winodict,
                        int(shots),
                        strategy,
                    ...
                    counter += 1
                    logging.info(
                        "%d/%d: Done writing %s_%sshot", counter, total, task, shots
                    )
        utils.merge_files(
